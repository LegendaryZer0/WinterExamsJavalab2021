...
function changeHead(n) {
    ...
    {
        console.log("Начато общение с "+n);
        $('#chat-right-header').text('Chat with ' + n);
...
function  getUrl(id_from,id_to) {
   ...
   $.ajax({
        ...
        success: function (data) {
            ...
            url =JSON.parse(JSON.stringify(data));
...
function changeChat(id_from, id_to, nick) {
    ...
    socket = new WebSocket("ws://localhost:8080/Semester_Work_war/chat/" + url);
    changeHead(nick);
...
/*    let s = hashCode(id_from + id_to + "")*/
    ...
    /*console.log("ws://localhost:8080/LabWork_war/chat/" + url);*/
