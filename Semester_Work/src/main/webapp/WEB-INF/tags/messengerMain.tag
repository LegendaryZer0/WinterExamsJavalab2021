...
$('#message-send-btn').click(function sendMessage(){

let val = $('#message-input').val()
let data = {
from :{id:main_id_from},
to :{id:main_id_to},
message: val,
...
console.log("Отправляю этот json")
console.log(data)
...
let  thisname = n
$('#chat-right-header').text('Chat with ' + thisname);
...
function changeChat(id_from,id_to,nick){
if(socket!==undefined){
console.log("Я вроде как закрыл старый сокет")
socket.close()
}
...
let s = hashCode(id_from+id_to+"")
console.log(s);
socket = new ru.itis.WebSocket("ws://localhost:8080/LabWork_war/chat/"+s);
console.log("ws://localhost:8080/LabWork_war/chat/"+s);
socket.onopen = function() {
var messages =JSON.parse(socket.data)
...
/* console.log("some liitle parse")*/
var a = JSON.parse(message.data);


/*         console.log("message");
console.log(message.data);
console.log("Json parse strigify");*/
...
function loadMesages(id_from,id_to) {
var data ={
id_to: id_to,
id_from: id_from
...
console.log("Это начальныый json c ajax запроса(сообщения)");
console.log(json);
console.log("Это json 0 ");
console.log("Э то последний json с ajax"+json[0]);
testJson=json[0];
$.each(json,function (index,message) {
if (message['from'].id === id_from){
$('#messageBox').append(" <div class=\"d-flex justify-content-start mb-4\">\n" +
"                        <div class=\"img_cont_msg\">\n" +
    "                            <img src=\"https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg\"\n" +
    "                                 class=\"rounded-circle user_img_msg\">\n" +
